...
@router.get("/definitions", response_model=list[SLADefinition])
async def api_list_sla_definitions() -> list[SLADefinition]:
    """列出 SLA 定义"""
    return list_sla_definitions()


@router.get("/definitions/{tier}", response_model=SLADefinition)
...
@router.get("/compliance", response_model=SLAStatus)
