...
class AppContext:
    ...
    async def initialize(self) -> None:
        ...
        from agent.core.approval_flow import ApprovalFlowManager
        self._approval_flow_manager = ApprovalFlowManager()
        logger.info("审批流管理器初始化完成")

        from agent.core.human_confirm import HumanConfirmManager
        ...
        from agent.core.feedback import FeedbackService
        self._feedback_service = FeedbackService()
        logger.info("反馈服务初始化完成")

        from agent.core.long_task import LongTaskManager
        self._long_task_manager = LongTaskManager()
        logger.info("长任务管理器初始化完成")

        from agent.core.message_queue import ScheduledTaskManager
        self._scheduled_task_manager = ScheduledTaskManager()
        logger.info("定时任务管理器初始化完成")

        from agent.core.task_checkpoint import TaskCheckpointStore
