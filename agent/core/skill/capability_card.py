...
使用方式：
    from agent.core.skill.capability_card import CapabilityCard, get_capability_registry
...
class CapabilityRegistry:
    ...
    def get_routing_for_intent(self, intent: str) -> dict[str, Any] | None:
        ...
        if not matched:
            # 未找到注册卡片的意图，对已知协作模式降级路由到 Supervisor
...
def _register_default_cards(registry: CapabilityRegistry) -> None:
    ...
    cards = [
        CapabilityCard(
            ...
            category="supervisor",
            supported_intents=["greeting", "general_query", "help", "cross_system", "complex_task"],
            supported_actions=["route", "classify", "summarize", "coordinate", "delegate"],
            capability_keywords=["路由", "分类", "调度", "规划", "协调", "跨系统", "多任务"],
